...
Для стабильного старта загрузчик прыгает на фиксированный адрес `0x10000`, а линковка гарантирует, что там находится точка входа `_start` (иначе возможен перезапуск из-за выполнения не того кода).
